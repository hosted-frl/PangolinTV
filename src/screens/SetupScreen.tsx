...
export default function SetupScreen({ onDone }: { onDone: () => void }) {
  ...
  const save = async () => {
    if (!baseUrl || !apiKey) return safeAlert('Please fill Pangolin URL and API key');
    const old = await loadConfig();
    const cfg: Config = {
      baseUrl,
      apiKey,
      orgId: orgId || undefined,
      seerrUrl: old?.seerrUrl,
      seerrApiKey: old?.seerrApiKey,
    } as any;
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.sectionTitle}>Pangolin</Text>
      ...
      {/* Seerr settings moved to separate screen */}
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { marginTop: 6, fontSize: 14, fontWeight: '600', color: '#fff' },
